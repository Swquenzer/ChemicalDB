<?php 
	...
	$query = "UPDATE inventory i JOIN chemical c
				...
				SET i.Room=?, i.Location=?, i.ItemCount=?, i.Units=?, i.MftrID=?
				WHERE i.Room=? && i.Location=? && i.ItemCount=? && i.Units=? && c.Name=?
			 ";
	$stmt = $db->stmt_init();
	if(!$stmt->prepare($query)) {
		...
		$stmt->bind_param('ssisississ', $room, $loc, $quant, $unit, $mftrID, $_SESSION['room'], $_SESSION['loc'], $_SESSION['quant'], $_SESSION['unit'], $_SESSION['chem']);
		$stmt->execute();
